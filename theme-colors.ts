/*
 * Shared color theme, consumed by both tailwind.config.ts (the real utility
 * engine) and uno.config.ts (kept around only for its icon preset). Extracted
 * to one file so the two never drift apart.
 */
...
export const BOLT_ELEMENT_COLORS = {
  bolt: {
    elements: {
